	...
	{
            var character  = (this.charCodeAt(i)%7)/7.0;
	    
	    if (i +1 < this.length)
	    {
		character *=  (this.charCodeAt((i+1)))%5/5.0;
 	    }
//	    hash = ((hash << 3) - hash) + character;
	    hash = hash*0.8 + character;
	    

        }
        hash = Math.abs(hash)%1.0;
        return hash;
    }
    ...
    function getUserColor(username)
    {
	       ...
	       colors = hslToRgb(code, (code > 0.06 && code < 0.14) ? 0.8:0.5, (code > 0.14 && code < 0.22) ?0.35:0.5);
